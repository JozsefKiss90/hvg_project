...
async function logPosts() {
    ...
    const posts = await Posts.find().sort({ created: "desc" });
...
const runPrompt = async (cb) =>{
    ...
    const prompt3 = await cb()
    const response = await openai.createCompletion({
        model: "text-davinci-003",
        prompt: `give back a shorter version of the following text: ${prompt3}`,
        max_tokens: 3000,
        temperature: 0,
    })
